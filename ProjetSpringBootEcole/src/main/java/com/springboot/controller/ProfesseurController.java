...
public class ProfesseurController {
	...
	public String getAfficheMod(@PathVariable final Integer id, Model pmodel) {
		...
		if (pmodel.containsAttribute("profform") == false) {
			...
			profform.setDatenaissance(new SimpleDateFormat("yyyy-MM-dd").format(prof.getDatenaissance()));
	...
	public String ajoutProf(@Valid @ModelAttribute ProfesseurForm profform, BindingResult presult, Model pmodel) {
	...
	public String modifieProf(@Valid @ModelAttribute ProfesseurForm profform, BindingResult presult, Model pmodel) {
		if (!presult.hasErrors()) {
			try {
				Professeur prof = convertForm(profform);
