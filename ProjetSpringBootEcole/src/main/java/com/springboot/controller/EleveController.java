...
public class EleveController {
	...
	private Eleve convertForm(EleveForm eleveform) throws Exception {
		...
		Classe pclasse = serviceclasse.rechercheClasseId(Integer.valueOf(eleveform.getClasse()));
	...
	public String getAfficheMod(@PathVariable final Integer id, Model pmodel) {
		...
		if (pmodel.containsAttribute("eleveform") == false) {
			...
			eleveform.setClasse(String.valueOf(peleve.getClasse().getId()));
			eleveform.setNom(peleve.getNom());
			eleveform.setPrenom(peleve.getPrenom());
			eleveform.setDatenaissance(new SimpleDateFormat("yyyy-MM-dd").format(peleve.getDatenaissance()));
			pmodel.addAttribute("eleveform", eleveform);
		}
		return "eleves";
	}
	...
	public String ajoutClasse(@Valid @ModelAttribute(name = "eleveform") EleveForm eleveform, BindingResult presult,
			Model pmodel) {
	...
	public String modifieClasse(@Valid @ModelAttribute(name = "eleveform") EleveForm eleveform, BindingResult presult,
			Model pmodel) {
		if (!presult.hasErrors()) {
			try {
				Eleve peleve = convertForm(eleveform);
