...
public class MatiereController {
	...
	private Matiere convertForm(MatiereForm matiereform) throws Exception {
		Matiere pcmatiere = new Matiere();
		pcmatiere.setId(matiereform.getId());
		pcmatiere.setNom(matiereform.getNom());
		return pcmatiere;
	}
	...
	public String modifieClasse(@Valid @ModelAttribute(name = "matiereform") MatiereForm matiereform,
			BindingResult presult, Model pmodel) {
		if (!presult.hasErrors()) {
			try {
				Matiere pmatiere = convertForm(matiereform);
